...
fn main() {
    ...
    println!("=== Testing alternating emptiness, ordering, and contiguity ===\n");
    ...
    for (name, gamma, a, nu) in &params {
        ...
        for k in 3..=8 {
            ...
            let mut first_alt_fail = None;
            let mut first_ord_fail = None;
            let mut first_cont_fail = None;
            let mut ancestors_empty_at = None;

            for t in 0..num_stages {
                pl = stage(&pl, k, t, gamma, a, nu);

                // Check alternating emptiness (after stage t, check parity t+1)
                if first_alt_fail.is_none() && !check_alternating_empty(&pl, k, t) {
                    first_alt_fail = Some(t + 1);
                }

                // Check ordering
                if first_ord_fail.is_none() && !check_ordering(&pl, k) {
                    first_ord_fail = Some(t + 1);
                }

                // Check if ancestors empty
                ...
                // Check contiguity at finish level (only when ancestors empty)
                if first_cont_fail.is_none() && ancestors_empty_at.is_some() {
                    if !check_contiguous(&pl, k, finish_level) {
                        first_cont_fail = Some(t + 1);
                    }
                }
            }
            ...
            print!("  k={}: alt_empty={}, ",
                k,
                if first_alt_fail.is_none() { "OK".to_string() }
                else { format!("FAIL@t={}", first_alt_fail.unwrap()) });
            print!("ord={}, ",
                if first_ord_fail.is_none() { "OK".to_string() }
                else { format!("FAIL@t={}", first_ord_fail.unwrap()) });
            print!("ancestors_empty={}, ",
                if let Some(t) = ancestors_empty_at { format!("t={}", t) }
                else { "never".to_string() });
            println!("contiguous={}",
                if first_cont_fail.is_none() { "OK".to_string() }
                else { format!("FAIL@t={}", first_cont_fail.unwrap()) });
        }
        println!();
    }
}
