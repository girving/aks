...
theorem completeGraph_neighbor {n : ℕ} (v : Fin (n + 1)) (i : Fin n) :
    (completeGraph n).neighbor v i = v.succAbove i := by
  simp [RegularGraph.neighbor, completeGraph, complete_rot]

/-- For the complete graph, the walk operator gives the average over all
    *other* vertices: `(Wf)(v) = ((∑ f) - f v) / n`. -/
theorem walkCLM_completeGraph_apply {n : ℕ}
...
theorem walkCLM_sub_meanCLM_completeGraph {n : ℕ} (hn : n ≥ 1) :
...
theorem norm_one_sub_meanCLM {n : ℕ} (hn : n ≥ 2) :
    ‖1 - meanCLM n‖ = 1 := by
  set T := 1 - meanCLM n
  apply le_antisymm
