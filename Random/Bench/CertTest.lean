...
def main : IO UInt32 := do
  ...
  failures := failures + (← check "decode 0" (decodeBase85Nat (String.fromUTF8! (encodeBase85 0)) 0 == 0))
  failures := failures + (← check "decode 1" (decodeBase85Nat (String.fromUTF8! (encodeBase85 1)) 0 == 1))
  failures := failures + (← check "decode 42" (decodeBase85Nat (String.fromUTF8! (encodeBase85 42)) 0 == 42))
  failures := failures + (← check "decode 84" (decodeBase85Nat (String.fromUTF8! (encodeBase85 84)) 0 == 84))
  failures := failures + (← check "decode 85" (decodeBase85Nat (String.fromUTF8! (encodeBase85 85)) 0 == 85))

  -- decode at offset k=1
  let bytesOff := (encodeBase85 0).append (encodeBase85 42)
  failures := failures + (← check "decode at offset" (decodeBase85Nat (String.fromUTF8! bytesOff) 1 == 42))
  ...
  for u in testValues do
    let decoded := decodeBase85Nat (String.fromUTF8! (encodeBase85 u)) 0
    if decoded != u then
  ...
  failures := failures + (← check "positive 42"
    (decodeBase85Int (String.fromUTF8! (encodeBase85 42)) 0 == 42))
  failures := failures + (← check "negative -1"
    (decodeBase85Int (String.fromUTF8! (encodeBase85 4294967295)) 0 == -1))
  failures := failures + (← check "negative -100"
    (decodeBase85Int (String.fromUTF8! (encodeBase85 4294967196)) 0 == -100))
  failures := failures + (← check "max positive 2^31-1"
    (decodeBase85Int (String.fromUTF8! (encodeBase85 2147483647)) 0 == 2147483647))
  failures := failures + (← check "min negative -2^31"
    (decodeBase85Int (String.fromUTF8! (encodeBase85 2147483648)) 0 == -2147483648))
  ...
  let smallRotStr := String.fromUTF8! smallRot

  failures := failures + (← check "valid 4-cycle involution"
    (checkInvolution smallRotStr 4 2))
  failures := failures + (← check "wrong size rejected"
    (!checkInvolution (String.fromUTF8! ⟨#[33, 33]⟩) 4 2))
  ...
  failures := failures + (← check "broken involution rejected"
    (!checkInvolution (String.fromUTF8! brokenRot) 4 2))
  ...
  let rotData16 : String := ascii_file% "data/16/rot_map.b85"
  let certData16 : String := ascii_file% "data/16/cert_z.b85"
  ...
  failures := failures + (← check "n=16 involution valid"
    (checkInvolution rotData16 16 4))
  ...
  let certBytes16 := certData16.toByteArray
