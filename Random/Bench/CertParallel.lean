...
def checkPSDChunk (neighbors : Array Nat) (certStr : String)
  ...
  Id.run do
    ...
    for j in columns do
      ...
      for k in [:j+1] do
        let zk := decodeBase85Int certStr (colStart + k)
        zCol := zCol.set! k zk
        colSum := colSum + zk
        for p in [:d] do
...
def checkPSDCertificateParallel (neighbors : Array Nat) (certStr : String)
    (n d : Nat) (c₁ c₂ c₃ : Int) (numChunks : Nat := 4) : IO Bool := do
  if certStr.utf8ByteSize != n * (n + 1) / 2 * 5 then return false
  ...
  for c in [:nc] do
    ...
    if cols.size > 0 then
      let task := Task.spawn (prio := .dedicated) fun () =>
        checkPSDChunk neighbors certStr n d c₁ c₂ c₃ cols
...
def checkCertificateParallel (rotStr certStr : String)
    (n d : Nat) (c₁ c₂ c₃ : Int) (numChunks : Nat := 4) : IO Bool := do
  if !checkInvolution rotStr n d then return false
  let neighbors := decodeNeighbors rotStr n d
  checkPSDCertificateParallel neighbors certStr n d c₁ c₂ c₃ numChunks
