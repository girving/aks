...
def rotData1728p : String := ascii_file% "data/1728/rot_map.b85"
def certData1728p : String := ascii_file% "data/1728/cert_z.b85"
...
def main : IO UInt32 := do
  ...
  -- No toUTF8 conversion needed - using String.getUTF8Byte for zero-copy access

  -- 2. Time checkInvolution alone
  IO.println "--- checkInvolution (n=1728, d=12) ---"
  timedP "checkInvolution " fun () =>
    s!"ok={checkInvolution rotData1728p n d}"
  IO.println ""
  ...
  timedP "mulAdj (ones)   " fun () => Id.run do
    ...
    let bz := mulAdj rotData1728p zOnes n d
    let mut s : Int := 0
  ...
  timedP "B²*ones         " fun () => Id.run do
    let zOnes := Array.replicate n (1 : Int)
    let bz := mulAdj rotData1728p zOnes n d
    let b2z := mulAdj rotData1728p bz n d
    let mut s : Int := 0
  ...
  timedP "checkPSD        " fun () =>
    s!"ok={checkPSDCertificate rotData1728p (b85entry certData1728p) n d c₁ c₂ c₃}"
  IO.println ""
  ...
  timedP "checkPSD (diag) " fun () => Id.run do
    ...
    for j in [:n] do
      ...
      for k in [:j+1] do
        zCol := zCol.set! k (decodeBase85Int certData1728p (colStart + k))
      let bz := mulAdj rotData1728p zCol n d
      let b2z := mulAdj rotData1728p bz n d
  ...
  timedP "decode all cert " fun () => Id.run do
    ...
    let mut s : Int := 0
    for k in [:total] do
      s := s + decodeBase85Int certData1728p k
  ...
  timedP "decode all rot  " fun () => Id.run do
    ...
    let mut s : Nat := 0
    for k in [:nd] do
      s := s + decodeBase85Nat rotData1728p (2 * k)
