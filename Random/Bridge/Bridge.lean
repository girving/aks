...
public import Random.Bridge.FastProof
public import Random.Bridge.ColumnNormBridge
public import Random.Bridge.WalkBound
public import Random.Bridge.SpectralMatrix
public import Random.Bridge.DiagDominant
...
noncomputable def certMatrixReal (entry : Nat → Nat → Int) (n : ℕ) :
...
/-- `allDiagPositive entry n = true` implies each diagonal entry is positive (as `ℤ`). -/
private theorem allDiagPositive_spec (entry : Nat → Nat → Int) (n : ℕ)
    (h : allDiagPositive entry n = true) (j : ℕ) (hj : j < n) :
    0 < entry j j := by
  induction n with
  | zero => omega
  | succ k ih =>
...
theorem certMatrix_posdiag (n : ℕ) (entry : Nat → Nat → Int) (rotBytes : String)
    (d : ℕ) (c₁ c₂ c₃ : ℤ)
    (hcert : checkPSDCertificate rotBytes entry n d c₁ c₂ c₃ = true) :
    ∀ j : Fin n, 0 < certMatrixReal entry n j j := by
  -- Extract allDiagPositive from checkPSDCertificate
  have h_diag : allDiagPositive entry n = true := by
  ...
  have hj_pos := allDiagPositive_spec entry n h_diag j.val j.isLt
...
private theorem certEntry_eq (entry : Nat → Nat → Int) (n : ℕ) (i j : Fin n) :
    certMatrixReal entry n i j = ↑(certEntryInt entry i.val j.val) := by
...
private theorem pEntry_eq {n d : ℕ} (G : RegularGraph n d)
    (rotBytes : String) (entry : Nat → Nat → Int) (hn : 0 < n) (hd : 0 < d)
    (hmatch : ∀ vp : Fin n × Fin d,
      G.rot vp = (⟨decodeBase85Nat rotBytes (2 * (vp.1.val * d + vp.2.val)) % n,
                    Nat.mod_lt _ hn⟩,
                  ⟨decodeBase85Nat rotBytes (2 * (vp.1.val * d + vp.2.val) + 1) % d,
                    Nat.mod_lt _ hd⟩))
    (c₁ c₂ c₃ : ℤ) (k j : Fin n) :
    (spectralMatrix G (↑c₁) (↑c₂) (↑c₃) * certMatrixReal entry n) k j =
    ↑(pEntryPure rotBytes entry n d c₁ c₂ c₃ k.val j.val) := by
  set Z := certMatrixReal entry n
  ...
  set zj_int : ℕ → ℤ := fun i ↦ certEntryInt entry i j.val
  set zj_real : Fin n → ℝ := fun l ↦ Z l j
  have hzj : ∀ l : Fin n, zj_real l = ↑(zj_int l.val) := fun l ↦ certEntry_eq entry n l j
  -- Bz correspondence
  ...
  -- Column sum
  have hColSum : ∑ l : Fin n, Z l j = ↑(sumTo (fun l ↦ certEntryInt entry l j.val) n) := by
    simp_rw [show ∀ l : Fin n, Z l j = ↑(certEntryInt entry l.val j.val) from
      fun l ↦ certEntry_eq entry n l j]
    rw [sumTo_eq_sum, Int.cast_sum]
  -- Combine
  rw [hI, hAA, hJ, hColSum]
  simp only [pEntryPure]
  have hZkj : Z k j = ↑(certEntryInt entry k.val j.val) := certEntry_eq entry n k j
...
private theorem kEntry_eq {n d : ℕ} (G : RegularGraph n d)
    ...
    (hmatch : ∀ vp : Fin n × Fin d,
      G.rot vp = (⟨decodeBase85Nat rotBytes (2 * (vp.1.val * d + vp.2.val)) % n,
                  ...
                  ⟨decodeBase85Nat rotBytes (2 * (vp.1.val * d + vp.2.val) + 1) % d,
                    Nat.mod_lt _ hd⟩))
    (c₁ c₂ c₃ : ℤ) (i j : Fin n) :
    (star (certMatrixReal entry n) * spectralMatrix G (↑c₁) (↑c₂) (↑c₃) *
      certMatrixReal entry n) i j =
    ↑(kEntryPure rotBytes entry n d c₁ c₂ c₃ i.val j.val) := by
  set Z := certMatrixReal entry n
  ...
  show ∑ k : Fin n, (star Z) i k * (M * Z) k j =
    ↑(sumTo (fun k ↦ certEntryInt entry k i.val *
      pEntryPure rotBytes entry n d c₁ c₂ c₃ k j.val) n)
  rw [sumTo_eq_sum, Int.cast_sum]
  apply Finset.sum_congr rfl
  intro k _
  ...
  rw [hstar]
  show certMatrixReal entry n k i * (M * certMatrixReal entry n) k j =
    ↑(certEntryInt entry k.val i.val * pEntryPure rotBytes entry n d c₁ c₂ c₃ k.val j.val)
  rw [certEntry_eq entry n k i,
      pEntry_eq G rotBytes entry hn hd hmatch c₁ c₂ c₃ k j]
  push_cast; ring
...
private theorem zColNormPure_eq_sum (entry : Nat → Nat → Int) (n j : Nat) :
    zColNormPure entry n j = ∑ k : Fin n, |certEntryInt entry k.val j| := by
...
private lemma epsMaxCol_entry_eq (rotBytes : String) (entry : Nat → Nat → Int) (n d : Nat) (c₁ c₂ c₃ : Int)
    (k j : Nat) :
    (let zj := fun i ↦ certEntryInt entry i j
     let b2zj_k := adjMulPure rotBytes (fun v ↦ adjMulPure rotBytes zj n d v) n d k
     c₁ * certEntryInt entry k j - c₂ * b2zj_k + c₃ * colSumZ entry n j) =
    pEntryPure rotBytes entry n d c₁ c₂ c₃ k j := by
  unfold pEntryPure colSumZ; rfl

private lemma epsMaxCol_bound (rotBytes : String) (entry : Nat → Nat → Int) (n d : Nat) (c₁ c₂ c₃ : Int)
    (j k bound : Nat) (hk : k < bound) :
    |pEntryPure rotBytes entry n d c₁ c₂ c₃ k j| ≤
    epsMaxCol rotBytes entry n d c₁ c₂ c₃ j (colSumZ entry n j) bound := by
...
private lemma epsMaxVal_bound (rotBytes : String) (entry : Nat → Nat → Int) (n d : Nat) (c₁ c₂ c₃ : Int)
    (k j bound : Nat) (hkj : k < j) (hjb : j < bound) :
    |pEntryPure rotBytes entry n d c₁ c₂ c₃ k j| ≤
    epsMaxVal rotBytes entry n d c₁ c₂ c₃ bound := by
  induction bound with
  | zero => omega
  | succ m ih =>
    unfold epsMaxVal
...
private lemma minDiagVal_bound (rotBytes : String) (entry : Nat → Nat → Int) (n d : Nat) (c₁ c₂ c₃ : Int)
    (j m : Nat) (hj : j < m) (hm : 0 < m) :
    minDiagVal rotBytes entry n d c₁ c₂ c₃ m ≤
...
private theorem checkPerRow_spec (entry : Nat → Nat → Int) (n : Nat) (ε mdpe : Int)
    (remaining i : Nat) (prefSum : Int)
    (h : checkPerRow entry n ε mdpe remaining i prefSum = true)
    (hpre : prefSum = sumTo (fun j ↦ zColNormPure entry n j) i)
    (k : Nat) (hki : i ≤ k) (hk : k < i + remaining) :
    entry k k * mdpe >
    ε * (sumTo (fun j ↦ zColNormPure entry n j) k +
         (↑(n - k) : Int) * zColNormPure entry n k) := by
  ...
  | succ r ih =>
    ...
    · have hi_lt_k : i + 1 ≤ k := by omega
      exact ih (i + 1) (prefSum + zColNormPure entry n i) hrest
...
private theorem checkColumnNormBoundPure_spec
    ...
    (hcnb : checkColumnNormBoundPure rotBytes entry n d c₁ c₂ c₃ = true) (hn : 0 < n) :
    ∃ epsMax minDiag : Int,
      (0 ≤ epsMax) ∧
      (∀ k j, k < j → j < n →
        |pEntryPure rotBytes entry n d c₁ c₂ c₃ k j| ≤ epsMax) ∧
      (∀ j, j < n → minDiag ≤ pEntryPure rotBytes entry n d c₁ c₂ c₃ j j) ∧
      (∀ i, i < n →
        entry i i * (minDiag + epsMax) >
        epsMax * (sumTo (fun j ↦ zColNormPure entry n j) i +
                  (↑(n - i) : Int) * zColNormPure entry n i)) := by
  -- Extract checkPerRow from the pure check
  unfold checkColumnNormBoundPure at hcnb
  rw [if_neg (by simp [beq_iff_eq]; omega)] at hcnb
  -- hcnb : checkPerRow entry n (epsMaxVal ...) (minDiagVal ... + epsMaxVal ...) n 0 0 = true
  -- Witness with epsMaxVal and minDiagVal
  set ε := epsMaxVal rotBytes entry n d c₁ c₂ c₃ n
  set δ := minDiagVal rotBytes entry n d c₁ c₂ c₃ n
  refine ⟨ε, δ, epsMaxVal_nonneg _ _ _ _ _ _ _ _,
          ...
          fun i hi ↦ ?_⟩
  exact checkPerRow_spec entry n ε (δ + ε) n 0 0 hcnb rfl i (Nat.zero_le _) (by omega)
    ...
    (hmatch : ∀ vp : Fin n × Fin d,
      G.rot vp = (⟨decodeBase85Nat rotBytes (2 * (vp.1.val * d + vp.2.val)) % n,
                  ...
                  ⟨decodeBase85Nat rotBytes (2 * (vp.1.val * d + vp.2.val) + 1) % d,
                    Nat.mod_lt _ hd⟩))
    (c₁ c₂ c₃ : ℤ) (i j : Fin n) :
    kEntryPure rotBytes entry n d c₁ c₂ c₃ i.val j.val =
    kEntryPure rotBytes entry n d c₁ c₂ c₃ j.val i.val := by
  -- Both sides cast to K[i,j] and K[j,i] via kEntry_eq
  have h1 := kEntry_eq G rotBytes entry hn hd hmatch c₁ c₂ c₃ i j
  have h2 := kEntry_eq G rotBytes entry hn hd hmatch c₁ c₂ c₃ j i
  -- K is Hermitian, so K[i,j] = K[j,i] for real matrices
  have hK := isHermitian_conjTranspose_mul_mul (certMatrixReal entry n)
    (spectralMatrix_isHermitian G ↑c₁ ↑c₂ ↑c₃)
  have hsym : (star (certMatrixReal entry n) * spectralMatrix G ↑c₁ ↑c₂ ↑c₃ *
    certMatrixReal entry n) i j =
    (star (certMatrixReal entry n) * spectralMatrix G ↑c₁ ↑c₂ ↑c₃ *
    certMatrixReal entry n) j i := by
...
private theorem checkAllRows_spec (rotBytes : String) (entry : Nat → Nat → Int) (n d : ℕ)
    (c₁ c₂ c₃ : ℤ) (m : ℕ)
    (h : checkAllRowsDomPure rotBytes entry n d c₁ c₂ c₃ m = true)
    (i : ℕ) (hi : i < m) :
    checkRowDomPure rotBytes entry n d c₁ c₂ c₃ i = true := by
  induction m with
  | zero => omega
  | succ m ih =>
...
theorem kRowDominant_implies_diagDominant
    (n d : ℕ) (hn : 0 < n) (hd : 0 < d)
    (G : RegularGraph n d)
    (rotStr : String) (entry : Nat → Nat → Int) (c₁ c₂ c₃ : ℤ)
    (hpure : checkAllRowsDomPure rotStr entry n d c₁ c₂ c₃ n = true)
    (hmatch : ∀ vp : Fin n × Fin d,
      G.rot vp = (⟨decodeBase85Nat rotStr (2 * (vp.1.val * d + vp.2.val)) % n,
                    Nat.mod_lt _ hn⟩,
                  ⟨decodeBase85Nat rotStr (2 * (vp.1.val * d + vp.2.val) + 1) % d,
                    Nat.mod_lt _ hd⟩))
    : let Z := certMatrixReal entry n
  ...
  have hrow := checkAllRows_spec rotStr entry n d c₁ c₂ c₃ n hpure i.val i.isLt
  ...
  have hentry : ∀ a b : Fin n,
      (star (certMatrixReal entry n) * spectralMatrix G ↑c₁ ↑c₂ ↑c₃ *
        certMatrixReal entry n) a b =
      ↑(kEntryPure rotBytes entry n d c₁ c₂ c₃ a.val b.val) :=
    fun a b ↦ kEntry_eq G rotBytes entry hn hd hmatch c₁ c₂ c₃ a b
  -- Rewrite diagonal
  rw [hentry i i]
  ...
  exact_mod_cast show ∑ j ∈ Finset.univ.erase i,
    ...
    rw [show ∑ j : Fin n, (if j = i then (0 : ℤ)
        ...
        sumTo (fun j ↦ if j == i.val then 0
          else let v := kEntryPure rotBytes entry n d c₁ c₂ c₃ i.val j
...
theorem congruence_diagDominant
    ...
    (G : RegularGraph n d)
    (rotStr : String) (entry : Nat → Nat → Int) (c₁ c₂ c₃ : ℤ)
    (hcert : checkCertificateSlowWith rotStr entry n d c₁ c₂ c₃ = true)
    (hcnbp : checkColumnNormBoundPure rotStr entry n d c₁ c₂ c₃ = true)
    (hmatch : ∀ vp : Fin n × Fin d,
      G.rot vp = (⟨decodeBase85Nat rotStr (2 * (vp.1.val * d + vp.2.val)) % n,
                    Nat.mod_lt _ hn⟩,
                  ⟨decodeBase85Nat rotStr (2 * (vp.1.val * d + vp.2.val) + 1) % d,
  ...
  intro _ _ i
  set rotBytes := rotStr
  -- Extract PSD sub-check from checkCertificateSlowWith
  have hpsd : checkPSDCertificate rotBytes entry n d c₁ c₂ c₃ = true := by
    simp only [checkCertificateSlowWith, Bool.and_eq_true] at hcert; exact hcert.1.2
  -- Get Z[i,i] > 0 from PSD check (reuse certMatrix_posdiag)
  have hZii_pos : 0 < certEntryInt entry i.val i.val := by
    have h := certMatrix_posdiag n entry rotBytes d c₁ c₂ c₃ hpsd i
  ...
  obtain ⟨ε, δ, hε_nn, hP_bound, hP_diag, hrow_check⟩ :=
    checkColumnNormBoundPure_spec rotBytes entry n d c₁ c₂ c₃ hcnbp hn
  -- Abbreviate column norm
  set S := zColNormPure entry n
  ...
  exact_mod_cast show ∑ j ∈ Finset.univ.erase i,
    ...
    have habs_bound : ∀ col row : ℕ, col < n → row < n → col < row →
        |∑ k : Fin n, certEntryInt entry k.val col *
          pEntryPure rotBytes entry n d c₁ c₂ c₃ k.val row| ≤
        ε * S col := by
      intro col row hcol hrow hlt
      calc |∑ k : Fin n, certEntryInt entry k.val col *
            pEntryPure rotBytes entry n d c₁ c₂ c₃ k.val row|
          ≤ ∑ k : Fin n, |certEntryInt entry k.val col| *
            |pEntryPure rotBytes entry n d c₁ c₂ c₃ k.val row| := by
            calc _ ≤ ∑ k : Fin n, |certEntryInt entry k.val col *
                  pEntryPure rotBytes entry n d c₁ c₂ c₃ k.val row| :=
                  Finset.abs_sum_le_sum_abs _ _
              _ = _ := by apply Finset.sum_congr rfl; intro k _; exact abs_mul _ _
        _ ≤ ∑ k : Fin n, |certEntryInt entry k.val col| * ε := by
    ...
    have hK_bound : ∀ j : Fin n, j ≠ i →
      ...
      · -- Case j < i: use K symmetry to swap, then col=j, row=i
        ...
        rw [Nat.min_eq_right (le_of_lt hji),
            kEntryPure_comm G rotBytes entry hn hd hmatch c₁ c₂ c₃ i j]
        unfold kEntryPure; rw [sumTo_eq_sum]
    ...
    have hoff : ∑ j ∈ Finset.univ.erase i,
        |kEntryPure rotBytes entry n d c₁ c₂ c₃ i.val j.val| + ε * S i.val ≤
    ...
    have hdiag : kEntryPure rotBytes entry n d c₁ c₂ c₃ i.val i.val ≥
        certEntryInt entry i.val i.val * (δ + ε) - S i.val * ε := by
      -- Express K[i,i] as sum, split at k = i
      have hsplit := Finset.add_sum_erase Finset.univ
        (fun k : Fin n ↦ certEntryInt entry k.val i.val *
          pEntryPure rotBytes entry n d c₁ c₂ c₃ k.val i.val)
        (Finset.mem_univ i)
      unfold kEntryPure; rw [sumTo_eq_sum, ← hsplit]
      -- Bound the i-th term: Z[i,i]*P[i,i] ≥ Z[i,i]*δ
      have hdiag_term : certEntryInt entry i.val i.val *
          pEntryPure rotBytes entry n d c₁ c₂ c₃ i.val i.val ≥
          certEntryInt entry i.val i.val * δ :=
      ...
      have hoff_abs : |∑ k ∈ Finset.univ.erase i,
          certEntryInt entry k.val i.val *
            pEntryPure rotBytes entry n d c₁ c₂ c₃ k.val i.val| ≤
          ε * (S i.val - certEntryInt entry i.val i.val) := by
        calc |∑ k ∈ Finset.univ.erase i, certEntryInt entry k.val i.val *
              pEntryPure rotBytes entry n d c₁ c₂ c₃ k.val i.val|
            ≤ ∑ k ∈ Finset.univ.erase i, |certEntryInt entry k.val i.val| *
              |pEntryPure rotBytes entry n d c₁ c₂ c₃ k.val i.val| := by
              calc _ ≤ ∑ k ∈ Finset.univ.erase i,
                    |certEntryInt entry k.val i.val *
                      pEntryPure rotBytes entry n d c₁ c₂ c₃ k.val i.val| :=
                    Finset.abs_sum_le_sum_abs _ _
                _ = _ := by apply Finset.sum_congr rfl; intro k _; exact abs_mul _ _
          _ ≤ ∑ k ∈ Finset.univ.erase i, |certEntryInt entry k.val i.val| * ε := by
          ...
          _ = ε * (S i.val - certEntryInt entry i.val i.val) := by
              rw [← Finset.sum_mul, mul_comm]; congr 1
              have herase := Finset.add_sum_erase Finset.univ
                (fun k : Fin n ↦ |certEntryInt entry k.val i.val|)
                (Finset.mem_univ i)
              rw [← zColNormPure_eq_sum] at herase
              have : |certEntryInt entry i.val i.val| =
                  certEntryInt entry i.val i.val := abs_of_pos hZii_pos
              linarith
    ...
    -- Bridge certEntryInt on diagonal to entry (upper triangular: certEntryInt entry i i = entry i i)
    have hcert_diag : certEntryInt entry i.val i.val = entry i.val i.val := by
      simp [certEntryInt]
    rw [hcert_diag] at hdiag
...
private theorem checkInvBelow_all {rotBytes : String} {n d m : Nat}
...
theorem checkInvolutionSpec_implies_rotFun_involution (rotStr : String) (n d : Nat)
    (hn : 0 < n) (hd : 0 < d)
    (h : checkInvolutionSpec (rotStr) n d = true) :
...
theorem checkInvolutionSpec_neighbor_lt (rotStr : String) (n d : Nat)
    (h : checkInvolutionSpec (rotStr) n d = true) :
    ∀ k, k < n * d → decodeBase85Nat rotStr (2 * k) < n := by
  unfold checkInvolutionSpec at h
  rw [Bool.and_eq_true] at h
  intro k hk
...
theorem checker_implies_spectralMatrix_psd
  ...
  have hK_dom : ∀ i : Fin n,
      ∑ j ∈ Finset.univ.erase i, ‖(star Z * M * Z) i j‖ <
      (star Z * M * Z) i i :=
    congruence_diagDominant n d hn hd G rotStr entry c₁ c₂ c₃ hcert hcnbp hmatch
...
theorem certificate_implies_walk_bound
  ...
  spectralMatrix_posSemidef_implies_walk_bound hn hd G ↑c₁ ↑c₂ ↑c₃
    (checker_implies_spectralMatrix_psd n d hn hd G rotStr entry c₁ c₂ c₃ hcert hcnbp hmatch)
    f hf
...
theorem certificate_bridge (n d : ℕ) (hn : 0 < n) (hd : 0 < d)
    ...
    (rotStr : String) (entry : Nat → Nat → Int) (c₁ c₂ c₃ : ℤ)
    (hcert : checkCertificateSlowWith rotStr entry n d c₁ c₂ c₃ = true)
    (hinvSpec : checkInvolutionSpec rotStr n d = true)
    (βn βd : ℕ) (hβd : 0 < βd)
    (hβ : c₁ * (↑βd * ↑βd) ≤ c₂ * (↑βn * ↑βn))
    (hc₁ : 0 ≤ c₁) (hc₂ : 0 < c₂)
    (hmatch : ∀ vp : Fin n × Fin d,
  ...
  have hcnb : checkColumnNormBound rotStr entry n d c₁ c₂ c₃ = true := by
    simp only [checkCertificateSlowWith, Bool.and_eq_true] at hcert; exact hcert.2
  have hcnbp : checkColumnNormBoundPure rotStr entry n d c₁ c₂ c₃ = true :=
  ...
  exact (spectralGap_le_of_walk_bound hd G (by exact_mod_cast hc₁) (by exact_mod_cast hc₂)
      (fun f hf ↦ certificate_implies_walk_bound n d hn hd G
        rotStr entry c₁ c₂ c₃ hcert hcnbp hmatch f hf)).trans
    (sqrt_coeff_le_frac (by exact_mod_cast hc₂) hd hβd (by
      have h : ((c₁ * (↑βd * ↑βd) : ℤ) : ℝ) ≤ ((c₂ * (↑βn * ↑βn) : ℤ) : ℝ) :=
        Int.cast_le.mpr hβ
      push_cast at h; nlinarith))

/-! **Base-85 specialization** -/

/-- `certificate_bridge` specialized to base-85 encoded certificate. -/
theorem certificate_bridge_b85 (n d : ℕ) (hn : 0 < n) (hd : 0 < d)
    (G : RegularGraph n d)
  ...
  certificate_bridge n d hn hd G rotStr (b85entry certStr) c₁ c₂ c₃
    hcert hinvSpec βn βd hβd hβ hc₁ hc₂ hmatch

/-- `certificate_bridge` specialized to base-128 encoded certificate. -/
theorem certificate_bridge_b128 (n d : ℕ) (hn : 0 < n) (hd : 0 < d)
    (G : RegularGraph n d)
    (rotStr certStr : String) (c₁ c₂ c₃ : ℤ) (scale : ℤ)
    (hcert : checkCertificateSlowWith rotStr (b128entry scale certStr)
      n d c₁ c₂ c₃ = true)
    (hinvSpec : checkInvolutionSpec rotStr n d = true)
    (βn βd : ℕ) (hβd : 0 < βd)
    (hβ : c₁ * (↑βd * ↑βd) ≤ c₂ * (↑βn * ↑βn))
    ...
    (hmatch : ∀ vp : Fin n × Fin d,
      G.rot vp = (⟨decodeBase85Nat rotStr (2 * (vp.1.val * d + vp.2.val)) % n,
                  ...
                  ⟨decodeBase85Nat rotStr (2 * (vp.1.val * d + vp.2.val) + 1) % d,
                    Nat.mod_lt _ hd⟩)) :
    spectralGap G ≤ ↑βn / (↑βd * ↑d) :=
  certificate_bridge n d hn hd G rotStr (b128entry scale certStr) c₁ c₂ c₃
    hcert hinvSpec βn βd hβd hβ hc₁ hc₂ hmatch

/-- `certificate_bridge` specialized to 5-byte base-128 encoded certificate. -/
theorem certificate_bridge_b128x5 (n d : ℕ) (hn : 0 < n) (hd : 0 < d)
    (G : RegularGraph n d)
    (rotStr certStr : String) (c₁ c₂ c₃ : ℤ) (scale : ℤ)
    (hcert : checkCertificateSlowWith rotStr (b128x5entry scale certStr)
      n d c₁ c₂ c₃ = true)
    (hinvSpec : checkInvolutionSpec rotStr n d = true)
    (βn βd : ℕ) (hβd : 0 < βd)
    (hβ : c₁ * (↑βd * ↑βd) ≤ c₂ * (↑βn * ↑βn))
    ...
    (hmatch : ∀ vp : Fin n × Fin d,
      G.rot vp = (⟨decodeBase85Nat rotStr (2 * (vp.1.val * d + vp.2.val)) % n,
                  ...
                  ⟨decodeBase85Nat rotStr (2 * (vp.1.val * d + vp.2.val) + 1) % d,
                    Nat.mod_lt _ hd⟩)) :
    spectralGap G ≤ ↑βn / (↑βd * ↑d) :=
  certificate_bridge n d hn hd G rotStr (b128x5entry scale certStr) c₁ c₂ c₃
    hcert hinvSpec βn βd hβd hβ hc₁ hc₂ hmatch
