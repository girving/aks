...
/-
  ...
  Imports `CertCheck` (definitions), `Random/Misc/ForLoop` (loop characterization),
  and `Mathlib.Tactic` for `set`, `ring`, etc.
-/

public import Random.Bridge.Defs
...
theorem mulAdjEntry_eq_getD (neighbors : Array Nat) (z : Array Int)
  ...
  unfold mulAdjEntry
  simp only [Id.run, bind, pure, Std.Legacy.Range.forIn_eq_forIn_range', Std.Legacy.Range.size, Nat.sub_zero,
...
/-- `zCol` (the `Array.ofFn` in `psdColumnStep`) produces
    `certEntryInt entry k j` for `k ≤ j` and `0` for `k > j`. -/
private theorem zCol_spec (entry : Nat → Nat → Int) (n j : Nat) (v : Nat) :
    (Array.ofFn fun (i : Fin n) =>
      if i.val ≤ j then entry j i.val
      else 0)[v]? =
    if v < n then some (certEntryInt entry v j) else none := by
  simp only [Array.getElem?_ofFn, certEntryInt]; split <;> rfl

/-- `getD` version of `zCol_spec`. -/
private theorem zCol_getD (entry : Nat → Nat → Int) (n j k : Nat) (hk : k < n) :
    (Array.ofFn fun (i : Fin n) =>
      if i.val ≤ j then entry j i.val
      else 0).getD k 0 =
    certEntryInt entry k j :=
  getD_of_getElem?_some (by rw [zCol_spec entry n j]; rw [if_pos hk])

/-- `colSum` (the column sum loop) equals `colSumZ entry n j`. -/
private theorem colSum_spec (entry : Nat → Nat → Int) (n j : Nat) (hj : j < n) :
    Nat.fold (j + 1) (fun k _ acc => acc +
      (Array.ofFn fun (i : Fin n) =>
        if i.val ≤ j then entry j i.val
        else 0).getD k 0) 0 =
    colSumZ entry n j := by
  rw [fold_sum_congr _ (fun k ↦ certEntryInt entry k j)
    (fun k hk ↦ zCol_getD entry n j k (by omega))]
  rw [← sumTo_eq_fold]
  simp only [colSumZ]
...
theorem decodeNeighbors_getD (rotBytes : String) (n d k : Nat) (hk : k < n * d) :
...
theorem mulAdjPre_getD_eq_adjMulPure
    (rotBytes : String) (z : Array Int) (n d v : Nat)
    (hv : v < n) (hz : z.size = n)
...
theorem adjMulPure_congr (rotBytes : String) (z₁ z₂ : Nat → Int) (n d v : Nat)
...
/-- The `pij` value computed inside `psdColumnStep` for row `i`, column `j` equals
    `pEntryPure rotBytes entry n d c₁ c₂ c₃ i j` (under involution). -/
private theorem psdColumnStep_pij_eq
    (rotBytes : String) (entry : Nat → Nat → Int) (n d : Nat) (c₁ c₂ c₃ : Int)
    ...
    pEntryPure rotBytes entry n d c₁ c₂ c₃ i j := by
  ...
  rw [hmulAdj_eq]
  -- Step 1: zCol[i]! = certEntryInt entry i j
  have hzCol_i : (zCol : Array Int).getD i 0 = certEntryInt entry i j :=
    zCol_getD entry n j i hi
  -- Step 2: colSum_imp = colSumZ entry n j = sumTo ... n
  have hcolSum : colSum_imp = sumTo (fun l ↦ certEntryInt entry l j) n := by
    change _ = colSumZ entry n j
    exact colSum_spec entry n j hj
  -- Step 3: bz.getD v 0 = adjMulPure rotBytes (certEntryInt entry · j) n d v for v < n
  have hbz : ∀ v, v < n → bz.getD v 0 =
      adjMulPure rotBytes (fun k ↦ certEntryInt entry k j) n d v := by
    intro v hv
    ...
    exact adjMulPure_congr rotBytes _ _ n d v (by omega)
      (fun k hk ↦ zCol_getD entry n j k hk)
  ...
  have hb2z_i : (mulAdjPre neighbors bz n d).getD i 0 = adjMulPure rotBytes
      (fun v ↦ adjMulPure rotBytes (fun k ↦ certEntryInt entry k j) n d v) n d i := by
  ...
  change c₁ * zCol.getD i 0 - c₂ * (mulAdjPre neighbors bz n d).getD i 0 + c₃ * colSum_imp =
    c₁ * certEntryInt entry i j -
    c₂ * adjMulPure rotBytes (fun v ↦ adjMulPure rotBytes (fun k ↦ certEntryInt entry k j) n d v) n d i +
    c₃ * sumTo (fun l ↦ certEntryInt entry l j) n
...
private def psdG (rotBytes : String) (entry : Nat → Nat → Int) (n d : Nat) (c₁ c₂ c₃ : Int)
    (j i : Nat) : Int :=
  let neighbors := decodeNeighbors rotBytes n d
  let zCol : Array Int := .ofFn fun (k : Fin n) =>
    if k.val ≤ j then entry j k.val else 0
...
private theorem psdG_eq_pEntryPure
    ...
    (j i : Nat) (hj : j < n) (hi : i < n)
    (hinv : ∀ k, k < n * d → decodeBase85Nat rotBytes (2 * k) < n) :
    psdG rotBytes entry n d c₁ c₂ c₃ j i =
    pEntryPure rotBytes entry n d c₁ c₂ c₃ i j := by
  simp only [psdG, Id.run, bind, pure, Std.Legacy.Range.forIn_eq_forIn_range', Std.Legacy.Range.size,
    Nat.div_one, Nat.sub_zero, Nat.add_sub_cancel, forIn_range'_eq_fold, Nat.zero_add]
  exact psdColumnStep_pij_eq rotBytes entry n d c₁ c₂ c₃ j i hj hi hinv
...
private theorem tracker_3var_eq_fold (g : Nat → Int) (j n : Nat) (e₀ d₀ : Int) (f₀ : Bool) :
  ...
  have hbody_ext : (fun i (r : MProd Int (MProd Bool Int)) =>
      (if (i == j) = true then
        ...
        else ForInStep.yield ⟨r.fst, r.snd.fst, r.snd.snd⟩ : Id (ForInStep _))) =
      (fun i r => (ForInStep.yield (trackerStepM g j i r) : Id (ForInStep _))) :=
    funext fun i => funext fun r => tracker_body_M g j i r
  conv in fun i r => _ => rw [hbody_ext]
  rw [forIn_range'_eq_fold (trackerStepM g j)]
...
private theorem foldMax_eq_epsMaxCol (rotBytes : String) (entry : Nat → Nat → Int) (n d : Nat)
    (c₁ c₂ c₃ : Int) (j k : Nat) :
    foldMax (fun i => pEntryPure rotBytes entry n d c₁ c₂ c₃ i j) k =
    epsMaxCol rotBytes entry n d c₁ c₂ c₃ j (colSumZ entry n j) k := by
  induction k with
...
private theorem psdColumnStepProd_eq_fold
    ...
    psdColumnStepProd (decodeNeighbors rotBytes n d) d
      entry n c₁ c₂ c₃ state j hj =
    Nat.fold (j + 1) (fun i _ s => trackerStepP
      (psdG rotBytes entry n d c₁ c₂ c₃ j) j i s)
      (state.epsMax, state.first, state.minDiag) := by
  -- Bridge forIn' (proven access) to forIn (! access), then apply tracker_3var_eq_fold.
  -- Step 1: Show psdColumnStepProd = the forIn-based do-block with psdG
  have h_to_forIn : psdColumnStepProd (decodeNeighbors rotBytes n d) d
      entry n c₁ c₂ c₃ state j hj =
    (Id.run do
      let mut epsMax := state.epsMax
      let mut first := state.first
      let mut minDiag := state.minDiag
      for i in [:j+1] do
        let pij := psdG rotBytes entry n d c₁ c₂ c₃ j i
        if i == j then
          if first then minDiag := pij; first := false
          else if pij < minDiag then minDiag := pij
        else if i < j then
          let absPij := if pij >= 0 then pij else -pij
          if absPij > epsMax then epsMax := absPij
      return (epsMax, first, minDiag)) := by
    unfold psdColumnStepProd
    simp only [Id.run, bind, pure]
    -- Bridge: show forIn' body with zCol[i] and colSum_proved
    -- equals forIn body with psdG (which uses zCol[i]! and colSum_!)
    -- Key: zCol is the same array, colSum computes the same value
    have hzs : (Array.ofFn fun (k : Fin n) =>
      if k.val ≤ j then entry j k.val
      else 0).size = n := Array.size_ofFn
    -- colSum with forIn' + proven = colSum with forIn + !
    have hcs : (forIn' (m := Id) [:j + 1] (0 : Int)
      (fun k (_ : k ∈ [:j + 1]) s => ForInStep.yield
      ...
      apply range_forIn'_eq_forIn
      intro i hi s; congr 1; congr 1
      rw [getElem!_pos _ i (by rw [hzs]; have := hi.upper; simp at this; omega)]
    -- Bridge main loop via range_forIn'_mprod_eq_forIn
    apply range_forIn'_mprod_eq_forIn
    intro i hi r
    -- Unfold psdG to expose its internals, then match LHS
    unfold psdG
    simp only [Id.run, bind, pure]
    -- Convert RHS from forIn + zCol[i]! to match LHS forIn' + zCol[i]
    rw [← hcs]
    -- Now only zCol[i] vs zCol[i]! remains
    have hib : i < (Array.ofFn fun (k : Fin n) =>
      if k.val ≤ j then entry j k.val
      else 0).size := by rw [hzs]; have := hi.upper; simp at this; omega
    rw [show (Array.ofFn fun (k : Fin n) =>
      if k.val ≤ j then entry j k.val
      else 0)[i]! = (Array.ofFn fun (k : Fin n) =>
      if k.val ≤ j then entry j k.val
      else 0)[i] from getElem!_pos _ i hib]
  -- Step 2: Apply tracker_3var_eq_fold
  rw [h_to_forIn]
  exact tracker_3var_eq_fold (psdG rotBytes entry n d c₁ c₂ c₃ j) j (j + 1)
    state.epsMax state.minDiag state.first
...
private theorem psdColumnStep_result
    ...
    (psdColumnStep (decodeNeighbors rotBytes n d) d
      entry n c₁ c₂ c₃ state j hj).epsMax = max state.epsMax
      (epsMaxCol rotBytes entry n d c₁ c₂ c₃ j (colSumZ entry n j) j) ∧
    (psdColumnStep (decodeNeighbors rotBytes n d) d
      entry n c₁ c₂ c₃ state j hj).minDiag =
      (if state.first then pEntryPure rotBytes entry n d c₁ c₂ c₃ j j
       else min state.minDiag (pEntryPure rotBytes entry n d c₁ c₂ c₃ j j)) ∧
    (psdColumnStep (decodeNeighbors rotBytes n d) d
      entry n c₁ c₂ c₃ state j hj).first = false := by
  -- Bridge PSDChunkResult → Prod (definitional equalities)
  show (psdColumnStepProd (decodeNeighbors rotBytes n d) d
      entry n c₁ c₂ c₃ state j hj).1 = _ ∧
    (psdColumnStepProd (decodeNeighbors rotBytes n d) d
      entry n c₁ c₂ c₃ state j hj).2.2 = _ ∧
    (psdColumnStepProd (decodeNeighbors rotBytes n d) d
      entry n c₁ c₂ c₃ state j hj).2.1 = _
  rw [psdColumnStepProd_eq_fold]
  -- Apply fold characterization (fold bound is j+1, so j < j+1 trivially)
  have hspec := trackerFold_spec (psdG rotBytes entry n d c₁ c₂ c₃ j) j (j + 1)
    state.epsMax state.minDiag state.first (by omega) he
  obtain ⟨heps, hfirst, hmin⟩ := hspec
  refine ⟨?_, ?_, hfirst⟩
  · -- epsMax: foldMax (psdG ...) j = epsMaxCol ... j
    rw [heps]; congr 1
    rw [foldMax_congr _ (fun i => pEntryPure rotBytes entry n d c₁ c₂ c₃ i j) j
      (fun i hi => psdG_eq_pEntryPure rotBytes entry n d c₁ c₂ c₃ j i hj (by omega) hinv)]
    exact foldMax_eq_epsMaxCol rotBytes entry n d c₁ c₂ c₃ j j
  · -- minDiag: psdG j j = pEntryPure j j
    rw [hmin, psdG_eq_pEntryPure rotBytes entry n d c₁ c₂ c₃ j j hj hj hinv]
...
private def stepFn (rotBytes : String) (entry : Nat → Nat → Int) (n d : Nat) (c₁ c₂ c₃ : Int)
    (state : PSDChunkResult) (j : Nat) : PSDChunkResult :=
  if hj : j < n then
    psdColumnStep (decodeNeighbors rotBytes n d) d entry n c₁ c₂ c₃ state j hj
  else state

/-- `stepFn` equals `psdColumnStep` when `j < n`. -/
private theorem stepFn_eq (rotBytes : String) (entry : Nat → Nat → Int) (n d : Nat) (c₁ c₂ c₃ : Int)
    (state : PSDChunkResult) (j : Nat) (hj : j < n) :
    stepFn rotBytes entry n d c₁ c₂ c₃ state j =
    psdColumnStep (decodeNeighbors rotBytes n d) d entry n c₁ c₂ c₃ state j hj := by
  simp only [stepFn, dif_pos hj]

/-- `psdColumnStep_result` restated with `stepFn` in the conclusion for `rw` matching. -/
private theorem stepFn_result (rotBytes : String) (entry : Nat → Nat → Int) (n d : Nat) (c₁ c₂ c₃ : Int)
    (state : PSDChunkResult) (j : Nat) (hj : j < n)
    (hinv : ∀ k, k < n * d → decodeBase85Nat rotBytes (2 * k) < n)
    (he : 0 ≤ state.epsMax) :
    (stepFn rotBytes entry n d c₁ c₂ c₃ state j).epsMax = max state.epsMax
      (epsMaxCol rotBytes entry n d c₁ c₂ c₃ j (colSumZ entry n j) j) ∧
    (stepFn rotBytes entry n d c₁ c₂ c₃ state j).minDiag =
      (if state.first then pEntryPure rotBytes entry n d c₁ c₂ c₃ j j
       else min state.minDiag (pEntryPure rotBytes entry n d c₁ c₂ c₃ j j)) ∧
    (stepFn rotBytes entry n d c₁ c₂ c₃ state j).first = false := by
  rw [stepFn_eq _ _ _ _ _ _ _ _ _ hj]
  exact psdColumnStep_result rotBytes entry n d c₁ c₂ c₃ state j hj hinv he

/-! **Sequential fold produces `epsMaxVal`/`minDiagVal`** -/

theorem epsMaxVal_nonneg (rotBytes : String) (entry : Nat → Nat → Int) (n d : Nat)
    (c₁ c₂ c₃ : Int) (m : Nat) :
    0 ≤ epsMaxVal rotBytes entry n d c₁ c₂ c₃ m := by
  induction m with
  | zero => simp [epsMaxVal]
  | succ k ih =>
    simp only [epsMaxVal]
...
private theorem minDiagVal_succ (rotBytes : String) (entry : Nat → Nat → Int) (n d : Nat)
    (c₁ c₂ c₃ : Int) (m : Nat) (hm : 1 ≤ m) :
    minDiagVal rotBytes entry n d c₁ c₂ c₃ (m + 1) =
    min (minDiagVal rotBytes entry n d c₁ c₂ c₃ m)
        (pEntryPure rotBytes entry n d c₁ c₂ c₃ m m) := by
  cases m with
  | zero => omega
...
private theorem epsMaxVal_mono (rotBytes : String) (entry : Nat → Nat → Int) (n d : Nat)
    (c₁ c₂ c₃ : Int) (j : Nat) :
    epsMaxVal rotBytes entry n d c₁ c₂ c₃ j ≤
    epsMaxVal rotBytes entry n d c₁ c₂ c₃ (j + 1) := by
  simp only [epsMaxVal]; exact Int.le_max_left _ _

/-- `epsMaxVal` is monotone in its bound argument. -/
private theorem epsMaxVal_le_of_le (rotBytes : String) (entry : Nat → Nat → Int) (n d : Nat)
    (c₁ c₂ c₃ : Int) {a b : Nat} (hab : a ≤ b) :
    epsMaxVal rotBytes entry n d c₁ c₂ c₃ a ≤
    epsMaxVal rotBytes entry n d c₁ c₂ c₃ b := by
  induction hab with
  | refl => exact Int.le_refl _
  | step _ ih => exact Int.le_trans ih (epsMaxVal_mono _ _ _ _ _ _ _ _)

/-- `epsMaxCol j ≤ epsMaxVal n` for `j < n`. -/
private theorem epsMaxCol_le_epsMaxVal (rotBytes : String) (entry : Nat → Nat → Int) (n d : Nat)
    (c₁ c₂ c₃ : Int) (j : Nat) (hj : j < n) :
    epsMaxCol rotBytes entry n d c₁ c₂ c₃ j (colSumZ entry n j) j ≤
    epsMaxVal rotBytes entry n d c₁ c₂ c₃ n := by
  have h1 : epsMaxCol rotBytes entry n d c₁ c₂ c₃ j (colSumZ entry n j) j ≤
      epsMaxVal rotBytes entry n d c₁ c₂ c₃ (j + 1) := by
...
private theorem minDiagVal_mono (rotBytes : String) (entry : Nat → Nat → Int) (n d : Nat)
    (c₁ c₂ c₃ : Int) (j : Nat) (hj : 1 ≤ j) :
    minDiagVal rotBytes entry n d c₁ c₂ c₃ (j + 1) ≤
    minDiagVal rotBytes entry n d c₁ c₂ c₃ j := by
...
private theorem minDiagVal_antitone (rotBytes : String) (entry : Nat → Nat → Int) (n d : Nat)
    (c₁ c₂ c₃ : Int) {a b : Nat} (ha : 1 ≤ a) (hab : a ≤ b) :
    minDiagVal rotBytes entry n d c₁ c₂ c₃ b ≤
    minDiagVal rotBytes entry n d c₁ c₂ c₃ a := by
  induction hab with
  | refl => exact Int.le_refl _
...
private theorem minDiagVal_le_pEntryPure (rotBytes : String) (entry : Nat → Nat → Int) (n d : Nat)
    (c₁ c₂ c₃ : Int) (j : Nat) (hj : j < n) :
    minDiagVal rotBytes entry n d c₁ c₂ c₃ n ≤
    pEntryPure rotBytes entry n d c₁ c₂ c₃ j j := by
  have h1 : minDiagVal rotBytes entry n d c₁ c₂ c₃ (j + 1) ≤
      pEntryPure rotBytes entry n d c₁ c₂ c₃ j j := by
    cases j with
...
private theorem checkPSDColumns_eq_listFoldl (rotBytes : String)
    (entry : Nat → Nat → Int) (n d : Nat) (c₁ c₂ c₃ : Int) (cols : Array (Fin n)) :
    checkPSDColumns (decodeNeighbors rotBytes n d) entry n d c₁ c₂ c₃ cols =
    (cols.toList.map Fin.val).foldl (stepFn rotBytes entry n d c₁ c₂ c₃)
  ...
  suffices ∀ (l : List (Fin n)) (init : PSDChunkResult),
      (forIn (m := Id) l init (fun (jf : Fin n) s =>
        ForInStep.yield (psdColumnStep (decodeNeighbors rotBytes n d) d
          entry n c₁ c₂ c₃ s jf.val jf.isLt))) =
      (l.map Fin.val).foldl (stepFn rotBytes entry n d c₁ c₂ c₃) init from
    this cols.toList _
  intro l
  induction l with
  ...
  | cons jf rest ih =>
    ...
    rw [ih]
    congr 1
    exact (stepFn_eq rotBytes entry n d c₁ c₂ c₃ init jf.val jf.isLt).symm
...
private theorem foldl_step_epsMax_mono
    ...
    state.epsMax ≤ (L.foldl (stepFn rotBytes entry n d c₁ c₂ c₃) state).epsMax := by
  induction L generalizing state with
  | nil => simp [List.foldl]
  | cons j rest ih =>
    simp only [List.foldl]
    have hj := hL j (.head rest)
    have hpsd := stepFn_result rotBytes entry n d c₁ c₂ c₃ state j hj hinv he
    have he' : 0 ≤ (stepFn rotBytes entry n d c₁ c₂ c₃ state j).epsMax := by
      rw [hpsd.1]; exact Int.le_trans he (Int.le_max_left _ _)
    calc state.epsMax
        ≤ max state.epsMax _ := Int.le_max_left _ _
      _ = (stepFn rotBytes entry n d c₁ c₂ c₃ state j).epsMax := hpsd.1.symm
...
private theorem foldl_step_minDiag_mono
    ...
    (L.foldl (stepFn rotBytes entry n d c₁ c₂ c₃) state).minDiag ≤ state.minDiag := by
  induction L generalizing state with
  | nil => simp [List.foldl]
  | cons j rest ih =>
    simp only [List.foldl]
    ...
    have he' : 0 ≤ (stepFn rotBytes entry n d c₁ c₂ c₃ state j).epsMax := by
      rw [hpsd.1]; exact Int.le_trans he (Int.le_max_left _ _)
    have hf' : (stepFn rotBytes entry n d c₁ c₂ c₃ state j).first = false := hpsd.2.2
    have h1 := ih _ (fun j hj => hL j (List.mem_cons_of_mem _ hj)) he' hf'
    have h2 : (stepFn rotBytes entry n d c₁ c₂ c₃ state j).minDiag ≤ state.minDiag := by
...
private theorem foldl_step_epsMax_le
    ...
    (hinv : ∀ k, k < n * d → decodeBase85Nat rotBytes (2 * k) < n)
    (L : List Nat) (state : PSDChunkResult) (hL : ∀ j ∈ L, j < n)
    (he : 0 ≤ state.epsMax) (hle : state.epsMax ≤ epsMaxVal rotBytes entry n d c₁ c₂ c₃ n) :
    (L.foldl (stepFn rotBytes entry n d c₁ c₂ c₃) state).epsMax ≤
      epsMaxVal rotBytes entry n d c₁ c₂ c₃ n := by
  induction L generalizing state with
  ...
  | cons j rest ih =>
    ...
    have he' : 0 ≤ (stepFn rotBytes entry n d c₁ c₂ c₃ state j).epsMax := by
      rw [hpsd.1]; exact Int.le_trans he (Int.le_max_left _ _)
    have hle' : (stepFn rotBytes entry n d c₁ c₂ c₃ state j).epsMax ≤
        epsMaxVal rotBytes entry n d c₁ c₂ c₃ n := by
...
private theorem foldl_step_epsMax_ge
    ...
    {j : Nat} (hj : j ∈ L) :
    epsMaxCol rotBytes entry n d c₁ c₂ c₃ j (colSumZ entry n j) j ≤
    (L.foldl (stepFn rotBytes entry n d c₁ c₂ c₃) state).epsMax := by
  induction L generalizing state with
  | nil => nomatch hj
  | cons x rest ih =>
    simp only [List.foldl]
    have hx := hL x (.head rest)
    have hpsd := stepFn_result rotBytes entry n d c₁ c₂ c₃ state x hx hinv he
    have he' : 0 ≤ (stepFn rotBytes entry n d c₁ c₂ c₃ state x).epsMax := by
      rw [hpsd.1]; exact Int.le_trans he (Int.le_max_left _ _)
...
private theorem foldl_step_not_first
    ...
    (L : List Nat) (state : PSDChunkResult) (hL : ∀ j ∈ L, j < n) (he : 0 ≤ state.epsMax)
    (hne : L ≠ []) :
    (L.foldl (stepFn rotBytes entry n d c₁ c₂ c₃) state).first = false := by
  cases L with
...
private theorem foldl_step_minDiag_ge
    ...
    (L : List Nat) (state : PSDChunkResult) (hL : ∀ j ∈ L, j < n) (he : 0 ≤ state.epsMax)
    (hne : L ≠ [])
    (hge : state.first = true ∨ minDiagVal rotBytes entry n d c₁ c₂ c₃ n ≤ state.minDiag) :
    minDiagVal rotBytes entry n d c₁ c₂ c₃ n ≤
    (L.foldl (stepFn rotBytes entry n d c₁ c₂ c₃) state).minDiag := by
  induction L generalizing state with
  | nil => exact absurd rfl hne
  | cons x rest ih =>
    simp only [List.foldl]
    ...
    have he' : 0 ≤ (stepFn rotBytes entry n d c₁ c₂ c₃ state x).epsMax := by
      rw [hpsd.1]; exact Int.le_trans he (Int.le_max_left _ _)
    have hmin_ge : minDiagVal rotBytes entry n d c₁ c₂ c₃ n ≤
        (stepFn rotBytes entry n d c₁ c₂ c₃ state x).minDiag := by
      rw [hpsd.2.1]
      cases hge with
      | inl hfirst =>
...
private theorem foldl_step_minDiag_le
    ...
    (L : List Nat) (state : PSDChunkResult) (hL : ∀ j ∈ L, j < n) (he : 0 ≤ state.epsMax)
    {j : Nat} (hj : j ∈ L) :
    (L.foldl (stepFn rotBytes entry n d c₁ c₂ c₃) state).minDiag ≤
    pEntryPure rotBytes entry n d c₁ c₂ c₃ j j := by
  induction L generalizing state with
  | nil => nomatch hj
  | cons x rest ih =>
    ...
    | inl hjx =>
      subst hjx
      have hstep_le : (stepFn rotBytes entry n d c₁ c₂ c₃ state j).minDiag ≤
          pEntryPure rotBytes entry n d c₁ c₂ c₃ j j := by
        rw [hpsd.2.1]
...
/-! **roundRobinPartition partition** -/

/-- Nat version of the column-list fold, used only for bridge proofs. -/
def roundRobinPartitionNat (n numChunks : Nat) : Array (Array Nat) :=
...
/-- `roundRobinPartitionNat` is literally the Nat.fold expression. -/
theorem roundRobinPartitionNat_eq_fold (n numChunks : Nat) :
    roundRobinPartitionNat n numChunks =
    Nat.fold n (fun j _ lists => lists.set! (j % numChunks) (lists[j % numChunks]!.push j))
...
/-- Every `j < n` appears in `roundRobinPartitionNat n numChunks` at chunk `j % numChunks`. -/
theorem roundRobinPartitionNat_mem (n numChunks j : Nat) (hj : j < n) (hnc : 0 < numChunks) :
    j ∈ (roundRobinPartitionNat n numChunks)[j % numChunks]!.toList := by
  rw [roundRobinPartitionNat_eq_fold]
...
private theorem fold_split_count {α : Type} (a b : Nat) (f : Nat → α → α) (init : α) :
  ...
  | zero => simp [Nat.fold_zero]
  | succ b ih =>
    simp only [Nat.fold_succ, Nat.add_succ]
    exact congrArg (f (a + b)) ih
...
private theorem bcl_fold_entry (n numChunks : Nat) (hnc : 0 < numChunks) (k : Nat) :
  ...
  | zero =>
    ...
    intro c hc
    simp only [countRes', Nat.fold_zero]
...
theorem roundRobinPartitionNat_entry (n numChunks j : Nat) (hj : j < n) (hnc : 0 < numChunks) :
    (roundRobinPartitionNat n numChunks)[j % numChunks]![j / numChunks]! = j := by
  rw [roundRobinPartitionNat_eq_fold]
...
/-- `roundRobinPartitionNat` has `numChunks` entries. -/
theorem roundRobinPartitionNat_size (n numChunks : Nat) (hnc : 0 < numChunks) :
    (roundRobinPartitionNat n numChunks).size = numChunks := by
  rw [roundRobinPartitionNat_eq_fold]
  exact (bcl_fold_invariant n numChunks hnc n).1

set_option maxHeartbeats 800000 in
/-- `roundRobinPartition` has `numChunks` entries. -/
theorem roundRobinPartition_size (n numChunks : Nat) :
    (roundRobinPartition n numChunks).size = numChunks := by
  unfold roundRobinPartition
  simp only [Id.run, bind, pure]
  rw [Std.Legacy.Range.forIn'_eq_forIn'_range']
  apply @List.forIn'_yield_preserves Nat (Array (Array (Fin n)))
    (fun arr => arr.size = numChunks)
  · exact Array.size_replicate
  · intro a _ arr harr
    show (arr.setIfInBounds _ _).size = numChunks
    rw [Array.size_setIfInBounds]; exact harr

/-- All entries in chunk `c` of `roundRobinPartitionNat n numChunks` are `< n`. -/
theorem roundRobinPartitionNat_bound (n numChunks c : Nat)
    (hnc : 0 < numChunks) (x : Nat)
    (hx : x ∈ (roundRobinPartitionNat n numChunks)[c]!.toList) (hc : c < numChunks) :
    x < n := by
  rw [roundRobinPartitionNat_eq_fold] at hx
...
theorem roundRobinPartitionNat_inner_bound (n numChunks j : Nat) (hj : j < n) (hnc : 0 < numChunks) :
    j / numChunks < (roundRobinPartitionNat n numChunks)[j % numChunks]!.size := by
  rw [roundRobinPartitionNat_eq_fold]
...
/-- Relation for `roundRobinPartition_map_val`: Fin array and Nat array agree under `map Fin.val`. -/
private def bclRel (nc : Nat) {n : Nat}
    (finArr : Array (Array (Fin n))) (natArr : Array (Array Nat)) : Prop :=
  finArr.size = nc ∧ natArr.size = nc ∧
  ∀ c, c < nc → (finArr[c]!).toList.map Fin.val = (natArr[c]!).toList

/-- `getElem!` after `setIfInBounds` at same index (polymorphic). -/
private theorem getElem!_setIB_eq {α : Type} [Inhabited α] (a : Array α) (i : Nat) (v : α)
    (h : i < a.size) : (a.setIfInBounds i v)[i]! = v := by
  simp only [Array.setIfInBounds, h, dite_true]
  show (a.set i v h).getD i default = v
  unfold Array.getD; simp [Array.size_set, h]

/-- `getElem!` after `setIfInBounds` at different index (polymorphic). -/
private theorem getElem!_setIB_ne {α : Type} [Inhabited α] (a : Array α) (i j : Nat) (v : α)
    (hij : i ≠ j) : (a.setIfInBounds i v)[j]! = a[j]! := by
  unfold Array.setIfInBounds
  split
  · next h =>
    show (a.set i v h).getD j default = a.getD j default
    unfold Array.getD; simp [Array.size_set]
    split
    · next hj => simp [Array.getElem_set, hij]
    · rfl
  · rfl

set_option maxHeartbeats 1600000 in
/-- Bridge: mapping `Fin.val` over chunk `c` of `roundRobinPartition` gives the corresponding
    `roundRobinPartitionNat` chunk. -/
theorem roundRobinPartition_map_val (n numChunks c : Nat)
    (hnc : 0 < numChunks) (hc : c < numChunks) :
    ((roundRobinPartition n numChunks)[c]!).toList.map Fin.val =
    (roundRobinPartitionNat n numChunks)[c]!.toList := by
  -- Unfold roundRobinPartition to forIn', convert to list form
  unfold roundRobinPartition
  simp only [Id.run, bind, pure]
  rw [Std.Legacy.Range.forIn'_eq_forIn'_range']
  simp only [Std.Legacy.Range.size, Nat.sub_zero, Nat.add_sub_cancel, Nat.div_one]
  -- Convert RHS Nat.fold to foldl over List.range'
  rw [roundRobinPartitionNat_eq_fold]
  have hfold_eq : Nat.fold n (fun j _ (lists : Array (Array Nat)) =>
      lists.set! (j % numChunks) (lists[j % numChunks]!.push j))
      (Array.replicate numChunks (Array.mkEmpty (n / numChunks + 1))) =
    (List.range' 0 n 1).foldl
      (fun lists a => lists.set! (a % numChunks) (lists[a % numChunks]!.push a))
      (Array.replicate numChunks (Array.mkEmpty (n / numChunks + 1))) := by
    rw [List.foldl_range'_eq_fold]; simp [Nat.zero_add]
  rw [hfold_eq]
  -- Apply relational invariant: forIn' on finArr relates to foldl on natArr
  have hrel := List.forIn'_yield_rel (bclRel numChunks)
    (List.range' 0 n 1)
    (Array.replicate numChunks (Array.mkEmpty (n / numChunks + 1) : Array (Fin n)))
    (Array.replicate numChunks (Array.mkEmpty (n / numChunks + 1) : Array Nat))
    (by
      refine ⟨Array.size_replicate, Array.size_replicate, ?_⟩
      intro c' hc'
      show ((Array.replicate numChunks (Array.mkEmpty (n / numChunks + 1) : Array (Fin n))).getD c' default).toList.map Fin.val =
           ((Array.replicate numChunks (Array.mkEmpty (n / numChunks + 1) : Array Nat)).getD c' default).toList
      unfold Array.getD; simp [Array.size_replicate, hc', Array.getElem_replicate])
    (fun j h finArr =>
      finArr.set! (j % numChunks) (finArr[j % numChunks]!.push ⟨j, by
        have := List.mem_range'.mp h; omega⟩))
    (fun j natArr =>
      natArr.set! (j % numChunks) (natArr[j % numChunks]!.push j))
    (by
      intro j _ finArr natArr ⟨hfs, hns, hchunks⟩
      show bclRel numChunks
        (finArr.set! (j % numChunks) (finArr[j % numChunks]!.push ⟨j, _⟩))
        (natArr.set! (j % numChunks) (natArr[j % numChunks]!.push j))
      refine ⟨?_, ?_, ?_⟩
      · show (finArr.setIfInBounds _ _).size = numChunks
        rw [Array.size_setIfInBounds]; exact hfs
      · show (natArr.setIfInBounds _ _).size = numChunks
        rw [Array.size_setIfInBounds]; exact hns
      · intro c' hc'
        show ((finArr.setIfInBounds (j % numChunks) _)[c']!).toList.map Fin.val =
          ((natArr.setIfInBounds (j % numChunks) _)[c']!).toList
        by_cases heq : j % numChunks = c'
        · subst heq
          rw [getElem!_setIB_eq finArr _ _ (by rw [hfs]; exact Nat.mod_lt j hnc)]
          rw [getElem!_setIB_eq natArr _ _ (by rw [hns]; exact Nat.mod_lt j hnc)]
          simp [Array.toList_push, hchunks _ (Nat.mod_lt j hnc)]
        · rw [getElem!_setIB_ne finArr _ _ _ heq]
          rw [getElem!_setIB_ne natArr _ _ _ heq]
          exact hchunks c' hc')
  exact hrel.2.2 c hc

/-- Coverage: `j` appears in `(roundRobinPartition n nc)[j%nc]!.toList.map Fin.val`. -/
theorem roundRobinPartition_mem_val (n numChunks j : Nat) (hj : j < n) (hnc : 0 < numChunks) :
    j ∈ ((roundRobinPartition n numChunks)[j % numChunks]!).toList.map Fin.val := by
  rw [roundRobinPartition_map_val n numChunks _ hnc (Nat.mod_lt j hnc)]
  exact roundRobinPartitionNat_mem n numChunks j hj hnc

/-- `roundRobinPartition` is a valid partition when `0 < tasks`. -/
theorem roundRobinPartition_valid (n tasks : Nat) (ht : 0 < tasks) :
    ValidPartition (roundRobinPartition n tasks) := by
  intro ⟨j, hj⟩
  refine ⟨j % tasks, ?_, ?_⟩
  · rw [roundRobinPartition_size n tasks]; exact Nat.mod_lt j ht
  · have hmem := roundRobinPartition_mem_val n tasks j hj ht
    rw [List.mem_map] at hmem
    obtain ⟨x, hx_mem, hx_val⟩ := hmem
    have : x = ⟨j, hj⟩ := Fin.ext hx_val
    rwa [this] at hx_mem

/-- Position `j / numChunks` is a valid index in chunk `j % numChunks` of `roundRobinPartition`. -/
theorem roundRobinPartition_inner_bound (n numChunks j : Nat) (hj : j < n) (hnc : 0 < numChunks) :
    j / numChunks < ((roundRobinPartition n numChunks)[j % numChunks]!).size := by
  have hmv := roundRobinPartition_map_val n numChunks _ hnc (Nat.mod_lt j hnc)
  have := congrArg List.length hmv
  simp only [List.length_map, Array.length_toList] at this
  rw [this]
  exact roundRobinPartitionNat_inner_bound n numChunks j hj hnc

/-- The `(j / numChunks)`-th entry in chunk `j % numChunks` of `roundRobinPartition` has value `j`. -/
theorem roundRobinPartition_entry_val (n numChunks j : Nat) (hj : j < n) (hnc : 0 < numChunks)
    (hib : j / numChunks < ((roundRobinPartition n numChunks)[j % numChunks]!).size :=
      roundRobinPartition_inner_bound n numChunks j hj hnc) :
    (((roundRobinPartition n numChunks)[j % numChunks]!).toList[j / numChunks]'(by
      rw [Array.length_toList]; exact hib)).val = j := by
  have hmv := roundRobinPartition_map_val n numChunks _ hnc (Nat.mod_lt j hnc)
  -- Use List.getElem? to avoid dependent-type rewriting
  have hinner_nat := roundRobinPartitionNat_inner_bound n numChunks j hj hnc
  -- (map Fin.val toList)[j/nc]? = natToList[j/nc]?
  have hget? : (((roundRobinPartition n numChunks)[j % numChunks]!).toList.map Fin.val)[j / numChunks]? =
      ((roundRobinPartitionNat n numChunks)[j % numChunks]!).toList[j / numChunks]? :=
    congrArg (·[j / numChunks]?) hmv
  -- LHS of hget? = some (toList[j/nc].val)
  have hlen : j / numChunks < (((roundRobinPartition n numChunks)[j % numChunks]!).toList.map Fin.val).length := by
    rw [List.length_map, Array.length_toList]; exact hib
  rw [List.getElem?_eq_getElem hlen, List.getElem_map] at hget?
  -- RHS of hget? = some j
  rw [List.getElem?_eq_getElem (by rw [Array.length_toList]; exact hinner_nat),
    Array.getElem_toList hinner_nat, ← getElem!_eq_getElem _ _ hinner_nat,
    roundRobinPartitionNat_entry n numChunks j hj hnc] at hget?
  exact Option.some.inj hget?

/-- `epsMaxVal` is bounded by any common upper bound on `epsMaxCol`. -/
private theorem epsMaxVal_le_of_bounds (rotBytes : String) (entry : Nat → Nat → Int) (n d : Nat)
    (c₁ c₂ c₃ : Int) (m : Nat) (b : Int) (hb : 0 ≤ b)
    (h : ∀ j, j < m → epsMaxCol rotBytes entry n d c₁ c₂ c₃ j (colSumZ entry n j) j ≤ b) :
    epsMaxVal rotBytes entry n d c₁ c₂ c₃ m ≤ b := by
  induction m with
...
private theorem le_minDiagVal_of_bounds (rotBytes : String) (entry : Nat → Nat → Int) (n d : Nat)
    (c₁ c₂ c₃ : Int) (m : Nat) (b : Int) (hm : 1 ≤ m)
    (h : ∀ j, j < m → b ≤ pEntryPure rotBytes entry n d c₁ c₂ c₃ j j) :
    b ≤ minDiagVal rotBytes entry n d c₁ c₂ c₃ m := by
  induction m with
  | zero => omega
  | succ k ih =>
    cases k with
...
private theorem merged_eq_sequential
    ...
    let columnLists := roundRobinPartition n 64
    ...
    merged.epsMax = epsMaxVal rotBytes entry n d c₁ c₂ c₃ n ∧
    merged.minDiag = minDiagVal rotBytes entry n d c₁ c₂ c₃ n ∧
  ...
  -- Key sizes
  have hcl_size : columnLists.size = 64 := roundRobinPartition_size n 64
  ...
  have hcl_getElem : ∀ i (hi : i < 64),
      (roundRobinPartition n 64)[i]! = columnLists[i]'(by rw [hcl_size]; exact hi) :=
  ...
  have hresult_eq : ∀ i (hi : i < results.size),
      results[i] = ((columnLists[i]'(by rw [hcl_size]; omega)).toList.map Fin.val).foldl
        (stepFn rotBytes entry n d c₁ c₂ c₃) init := by
    intro i hi
    show (columnLists.map _)[i] = _
    rw [Array.getElem_map]; exact checkPSDColumns_eq_listFoldl _ entry n d c₁ c₂ c₃ _
  ...
  have hcoverage : ∀ j, j < n →
    ...
    exact roundRobinPartition_mem_val n 64 j hj (by omega)
  ...
  have hchunk_eps_le : ∀ i (hi : i < results.size),
      results[i].epsMax ≤ epsMaxVal rotBytes entry n d c₁ c₂ c₃ n := by
    intro i hi; rw [hresult_eq i hi]
    exact foldl_step_epsMax_le rotBytes entry n d c₁ c₂ c₃ hinv _ _
  ...
  have hchunk0_first : results[0].first = false := by
    ...
    exact foldl_step_not_first rotBytes entry n d c₁ c₂ c₃ hinv _ _
  ...
  have hmin_le : (results.toList.foldl PSDChunkResult.merge init).minDiag ≤
      minDiagVal rotBytes entry n d c₁ c₂ c₃ n := by
...
theorem checkColumnNormBound_implies_pure
    (rotBytes : String) (entry : Nat → Nat → Int) (n d : Nat) (c₁ c₂ c₃ : Int)
    (h : checkColumnNormBound rotBytes entry n d c₁ c₂ c₃ = true)
    (hinv : ∀ k, k < n * d → decodeBase85Nat rotBytes (2 * k) < n)
    (hn : 0 < n) :
    checkColumnNormBoundPure rotBytes entry n d c₁ c₂ c₃ = true := by
  -- Get the merged values equality (heps/hmin/hfirst are in Array.foldl form)
  have hseq := merged_eq_sequential rotBytes entry n d c₁ c₂ c₃ hn hinv
  ...
  -- h: checkPerRow entry n merged.epsMax (merged.minDiag + merged.epsMax) n 0 0 = true
  ...
  simp only [hnn]
  -- Goal: checkPerRow entry n (epsMaxVal ...) (minDiagVal ... + epsMaxVal ...) n 0 0 = true
  rw [← heps, ← hmin]
  exact h
