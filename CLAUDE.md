...
**Substantial (months):** TreeSorting.lean sorrys (3). **Proved:** `halvers_give_bounded_nearsort` (composition of `buildRecursiveNearsort` + `recursive_nearsort_bounded_tree_damage` + size bound), `zigzag_decreases_wrongness_v2` (from `HasBoundedZigzagDamage` + anti-monotonicity). **Sorry:** `recursive_nearsort_bounded_tree_damage` (induction on recursion depth), `bounded_tree_damage_gives_zigzag` (cherry alternation: `HasBoundedTreeDamage` → `HasBoundedZigzagDamage`), `aks_tree_sorting` (top-level assembly). **Deleted (V1 orphans):** `register_reassignment_increases_wrongness`, `zigzag_decreases_wrongness`, `cherry_wrongness_after_nearsort` (V1), `zig_step_bounded_increase` (V1). Full audit: [`docs/treesorting-audit.md`](docs/treesorting-audit.md).
