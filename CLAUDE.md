...
**Substantial (months):** TreeSorting.lean sorrys (5): `halvers_give_bounded_nearsort` (bridge: halver → bounded damage), `epsilonNearsort_correct` (recursive nearsort), `register_reassignment_increases_wrongness` (Lemma 1, needs reformulation — `elementsAtDistance` doesn't use its `t` parameter, so time evolution is vacuous), `zigzag_decreases_wrongness` (Lemma 3, needs tree alternation structure), `aks_tree_sorting` (top-level assembly). **Done:** reformulation (HasBoundedDamage introduced), `cherry_wrongness_after_nearsort` (proved), `zig_step_bounded_increase` (proved), `displacement_from_wrongness` (proved), `fringe_amplification_bound` (deleted — orphaned, mathematically incorrect).
