...
- **Avoid non-terminal `simp` unless it makes proofs significantly cleaner.** A non-terminal `simp` is one that's not the final tactic closing the goal. These can be fragile when proofs are modified later, as adding new simp lemmas to Mathlib or the project can change what `simp` simplifies, breaking downstream tactics. Instead, use `simp only [specific, lemmas]` to control exactly what gets simplified, or `rw [lemma1, lemma2]` to be explicit. Exception: if the alternative is much uglier (many explicit rewrites that would all be handled by simp), then non-terminal simp is acceptable, but document why.
...
**Verify theorem statements against the source paper early.** Before building substantial infrastructure for a theorem, read the primary source to confirm your theorem statement matches what the paper actually proves. Check: (1) Is it a single application or repeated/recursive? (2) Are there tree structures, auxiliary state, or bookkeeping steps that are essential to the proof? (3) Do the definitions exactly match (not just intuitively)? Informal sources (lecture notes, Wikipedia, blog posts) provide intuition but can be misleading about the precise result. **Example from this project:** `halver_composition` was formulated as "single ε-halver improves sortedness" based on informal understanding. After building Phases 0-3 infrastructure, reading AKS (1983) revealed they prove geometric decrease from **recursive ε-nearsort** (many ε₁-halvers with ε₁ << ε), not single application. The tree structure is essential to their proof. Catching this mismatch early (during planning, not after implementation) saves weeks of work. Read primary sources at the design stage, not after getting stuck.
...
**Assess proof risk before significant work.** For non-trivial theorems (especially those requiring new techniques or connecting disparate concepts), explicitly assess risk before implementation. Break into phases with risk levels: LOW (definition, direct proof from definitions), MEDIUM (standard argument with uncertainty about details), HIGH (novel connection, unclear if approach works). Identify the highest-risk phase and consider alternatives upfront. Document the risk assessment and fallback plans (axiomatize, defer, reformulate) before starting. Track risk as you work: if a MEDIUM risk phase reveals unexpected difficulty after 2-3 attempts, escalate to user rather than continuing. **Example from this project:** Attempting to prove single ε-halver composition was HIGH risk (no paper proof, unclear mechanism). Analysis revealed it's likely impossible for Boolean sequences. Reformulation to iterated halvings reduced risk: phases B.1-B.2 LOW, B.3 MEDIUM-HIGH (has fallback: axiomatize if stuck), B.4-B.5 LOW. Always identify the critical bottleneck lemma and validate it's provable before building dependencies.

**Analyze uncertain lemmas in natural language before formal proof attempts.** When facing a non-trivial lemma where the proof strategy is unclear or the connection between concepts is subtle, spend time working through the mathematics in natural language with concrete examples BEFORE attempting formal proof. This is faster iteration: (1) Test the proof idea with specific numbers/examples, (2) Look for counterexamples that break your intuition, (3) Verify each step of the informal argument holds, (4) Only then formalize. Each formal proof attempt involves slow build-check cycles (20s - 2min), while informal analysis is instant. A 15-minute careful analysis can reveal that a lemma is unprovable (saving days of formal attempts) or clarify the exact proof structure needed (making formalization straightforward). **Example from this project:** Before attempting `halver_decreases_wrongness`, analysis with concrete example (alternating sequence [1,0,1,0,...]) revealed that balanced distribution doesn't imply reduced displacement - the lemma as stated is likely unprovable. This saved multiple failed proof attempts.
...
**Search Mathlib before writing custom helpers.** Before defining a helper function or writing a manual proof, check whether Mathlib already provides it — existing helpers come with simp lemmas, API, and composability that custom code won't have. This applies especially to `Fin` operations, order theory, and algebraic identities. Examples found so far: `Fin.succAbove`/`Fin.predAbove` (skip-one-value embeddings with involution lemmas), `Monotone.map_min`/`Monotone.map_max` (`Mathlib.Order.MinMax`). To search: (1) grep `.lake/packages/mathlib` for keywords (fastest for exact matches), (2) use `#check @Fin.someName` in a scratch file to test if a name exists, or (3) **use LeanSearch** (https://leansearch.net/) for semantic/natural language queries (e.g., "operator norm is top eigenvalue" → `LinearMap.IsSymmetric.hasEigenvalue_iSup_of_finiteDimensional`). LeanSearch is especially valuable when you know what you want mathematically but not the Mathlib name. **TODO:** Add a script to query LeanSearch programmatically from the CLI. Reparameterize types to match Mathlib conventions (e.g., `Fin (n+1)` instead of `Fin d` with `hd : d ≥ 2`).
...
**Nested if-then-else: manual `by_cases` over `split_ifs`.** When proving properties of functions with 2+ levels of if-then-else nesting (e.g., `Comparator.apply`), avoid `split_ifs` — it generates context-dependent hypothesis names that are fragile to reference. Instead: (1) use manual `by_cases h : condition` for each branch point, (2) immediately rewrite with `rw [if_pos h]` or `rw [if_neg h]` to simplify the if-then-else, (3) prove the resulting goal. This gives predictable goal states and named hypotheses you control. Build up facts with anonymous `have : ...` statements and reference them inline with `‹fact›` to avoid naming clutter. Use explicit `le_trans` chains instead of `calc` if types don't unify cleanly. Pattern: `by_cases h : a = c.i; · rw [if_pos h]; ...; · rw [if_neg h]; ...`.
