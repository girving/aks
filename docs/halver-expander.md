...
Note: The tree-based AKS correctness proof is in `TreeSorting.lean`, not here. The `expander_gives_halver` proof is in `Halver/FromExpander.lean`.
...
### `AKS/Halver/FromExpander.lean` — Expander → ε-Halver Bridge (~650 lines)
